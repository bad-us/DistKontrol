...
def read_file():
    tree = ET.fromstring("/home/bad/File/vutlan.xml")
    for page in list(tree):
        title = page.find('value').text
        content = page.find('categories').text
    ...
    print('title: %s; content: %s' % (title, content))
    # with open(f"/home/bad/File/item.txt", encoding="utf8") as file:  # Разбираем файл с ключами построчно
    #     for line in file:
    #         word = line.strip()
    #         read_file_xml(word)
...
def read_file_xml(word):
    with open(f"/home/bad/File/vutlan.txt", encoding="utf8") as file: # Разбираем файл с ключами построчно
        for line in file:
            if word in line:
                f = line.split(word)
                f = f[1]
